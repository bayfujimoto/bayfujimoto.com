...
### The papers desk (the desk is one open folder)

- decision: The homepage desk is **one open folder holding five bundles of documents clipped together**, one bundle per series, in place of the five containers (dossier, box, sphere, stamp-and-paper, bundle). The papers are the fonds; the things that are not paper — the Guide's key, an amber block, the stamp, and the clips — are the only objects, and they stand on the paper. Each bundle holds **one document per subcollection**, typeset from the archive (the CV, the timetable, the calling card, the log) or a real scan at true size (Accumulation, from each record's `dimensions`; a scan still on the red scanning ground never reaches the desk). Clicking a bundle lifts its documents into a row across the screen — a column on a phone — and the fan is the series layer at the series address; a document opens its browse; a flat series opens its grid directly. The folder and papers are a DOM layer between the wood and an objects canvas; the phone has its own portrait composition and the objects keep their size relative to the papers. Implemented 2026-09-07 in `src/app/desk-alt.js` / `src/styles/desk-alt.css`; the five-container desk is kept on the `original-desk-objects` branch. Plan and history: `docs/desk-papers-plan.md`; the earlier container essay `docs/desk-objects.md` stands as the record of what it replaced.
- reason: It realises the interaction model's founding sentence — an object lifts toward the camera and splits into its subcollection parts — literally; it gives the desk and the inspection card one grammar (recto, verso, reproduction, caption); it makes the key's difference categorical rather than positional; and, in a bundle of real ephemera, the desk shows the archive's own contents.
- open: the Guide's frames still describe the five containers and their models — to be rewritten for the bundles; the clips are CC-BY models (blacksheeptal, risteralline, rxf10240) and the site owes a credit line; the key's lift-and-turn is not yet wired into this desk; the amber block is a placeholder and its purpose unresolved.

### The desk in the scene, with two lights

- decision: The desk is rendered entirely in WebGL (`src/app/desk-scene.js`): the wooden desk with its own materials, the folder, every document as a plane whose texture is drawn from a spec (`src/app/desk-docs.js` → `src/app/paper.js`), and the objects and clips — one scene, real shadows. It has **two modes**, switched with the **Space key** on the desk (only there; only that key). **Light** is the overhead lamp as the desk has always been lit — a warm spot above the desk leaning toward the pointer. **Dark** is a handheld flashlight held low on an arc at the near edge, on the cursor's side, aimed at the point under the cursor, with a cookie on the spot, a volumetric cone, dust through the room lit by the beam, bloom on the hotspot, grain over the frame, and a blue-black palette for the layers above (`src/styles/desk-dark.css`, `[data-mode="dark"]`). The mode follows the system colour scheme on arrival. Going dark, the overhead goes out, a beat of darkness, then the flashlight stutters on; coming back, the flashlight clicks off, a beat, and the lamp flickers back. Reduced motion switches at once. Implemented 2026-09-07; the DOM papers desk (`desk-alt.js`) and the earlier look switch (`look.js`) are retired; `/home-alt` and `/home-metal` resolve to `/`.
- reason: Bay's realism pass on the flashlight (a real light print, dust through the whole room, a real surface for the beam to rake across, no lag) could not be met with papers in the DOM, so the papers moved into the scene; and once they were there, the lamp lit them better than the DOM overlay ever had. Two lights on one desk is cheaper and truer than two desks. Space alone as the switch is Bay's call (no tap or screen-reader path yet — see open).
- open: a tap-reachable switch for phones and a labelled control for screen readers; the fan's in-hand lighting is a flat rig rather than the desk's light; the Guide's frames still describe the five containers.

### Guide — inspection card of desk objects
- status: confirmed
- decision: The Guide opens as a catalog card in the photo card's grammar (`buildCardWrap`, Guide mode), not as a prose box. Its frames are the six desk objects in strip order — the key first, then identity, labor, consumption, creation, accumulation — each a still in the contact strip and, on the plate, the object's own 3D model (a trackball: drag to turn it freely in any direction, no pole and no fixed up; it holds still until touched — no idle rotation; the still stands in when WebGL is unavailable). The fields column is rebuilt per frame: `object` (what sits on the desk) and `type` (`meta` / `series`), `title`, `container` (printed only where it differs from the object), `holds`, `extent` (published record and subcollection counts), `model` (the GLB filename), and a `note`. The key frame's note is the intro (the body of `src/content/guide.md`); each object's note is its `description` from the file's front matter (`objects.<key>.description`), edited in the admin's Guide form. Each frame describes the series its object **opens** — the box frame is Accumulation and the bundle frame Labor, following `DESK_CLICK_REMAP` — so the Guide tells the truth about what clicking does. Frames are addressable at `/guide/<key>/` (the bare `/guide/` is the key); stepping replaces rather than pushes history, so Back leaves the Guide in one step. The card does not link onward to the series — the Guide describes the desk; the desk is where you open things. Thumbnails are pre-rendered by `scripts/render-desk-thumbnails.js` to `public/thumbnails/desk/<key>.png` and committed with the code. The six model filenames and the click remap now live in one shared table, `src/shared/desk-objects.js`, used by the scene, the card, the build, and the scripts.
...
### CV — inspection card with a timeline plate
- status: confirmed
- decision: The CV opens as one catalog card whose contact strip holds every `cv-entry` record, most recent first (`sort_date` order, all categories together), replacing the grouped document box. The plate is the calibrated plate with a **year scale** in place of millimetres (`src/app/timeline-plate.js`): time runs up the plate, every entry is a bar in its category's column (employment, education, … in a fixed order; overlapping bars step into lanes), the selected entry is lit and named by its mark, open-ended spans run dashed to a `now` rule drawn from the client clock, and clicking a bar steps the card. No zoom, no pan. Strip cells are typographic **tiles** — a short `mark` over the years — since these records have nothing to photograph; `mark` is an optional record field (authored on the six entries: LDO, Rice, RSAP, SHoP, Weiss, Rice) derived from the organization when absent (`deriveMark`, `field-schema.js`), and the build writes the resolved marks and the set's year range beside the items (`cv.marks`, `cv.range`). The fields column is rebuilt per frame and avoids repeating itself: `ID · type`, title, organization only where it is not already the title, role, category, and one `date` row carrying the recorded range with the derived duration appended (`Sept 2023 – June 2024 · 9 mo`; open spans `· ongoing`), then the note and riders. Entries are addressable at `/identity/cv/?item=<id>` — the archive's existing item address, so `related_ids` from anywhere land on a frame; the bare `/identity/cv/` is the most recent entry; stepping replaces history. Built on a generalised **frames mode** of `buildCardWrap` (`ctx.frames`: fields, plate, tile, onFrame) that the Guide now uses as well — no third inspection view.
- reason: The CV was the last records-backed surface outside the one inspection grammar. With nothing to scan, the honest thing for the plate to measure is time, in the plate's own dress. One card rather than a grid of cards because six spans of time attached to names are one document, legible only against each other; the grid-and-card-per-entry study (`mockups/cv-entries/02`) remains the promotion path if the CV outgrows a strip. Plan: `docs/cv-inspection-card-plan.md`; studies: `mockups/cv-entries/`.
- date: 2026-09-05
...
### Biography — a homed constellation of memorable items
- status: confirmed
- decision: The biography stops being a prose record. It is a **constellation** — a registry record at `src/content/constellations/biography.md` whose members are the archive's favorite or memorable items, joined from any series through the ordinary `constellations` chip field — and it is **homed** in the Identity dossier: its address stays `/identity/biography/` (breadcrumb desk / Identity / biography; `/constellations/biography/` redirects there), the view is the constellation grid as it stands (contact-sheet grid, year-grouped, newest first — no time spine, no authored order), and in a constellation grid every record keeps its own subcollection's treatment (photo pile, padded cover, disc, true-relative ephemera) and a **film shows its poster** — the object among objects; the ribbon alone stays backdrops (`scripts/enrich-film-posters.js` fills posters the CSV ingest never recorded, backdrop standing in as a still until then) — and the one-paragraph description moves out of the document and into the **layer-meta as the registry's note**, under the title and date (`overlay-subtitle--note`), where every constellation's voice already sits. Member cards do **not** print `biography` in their constellations rider row — the biography is reached through the dossier, not from its members (`CONSTELLATION_HOMES` in `src/app/router.js` declares the home; `homedConstellationSlug` resolves an address to its registry slug). The `biography` record type, its two BIO records, its template, and its admin form (`short_bio` / `long_bio`, roles, location, links) are retired; the Identity sheet counts the constellation's members for the subcollection.
- reason: A reader infers a life better from material evidence than from self-description (`docs/biography-format-options.md` set the premise; this takes the shortest of its paths, the existing constellation grid, rather than the time spine, and defers curation to the meta-object phase with every other constellation). One mechanism for membership — the registry — rather than an ordered `path` on the record, so the admin's chip field and the build's derived membership work unchanged. Homed rather than a plain constellation because the biography belongs to the dossier's three documents, and a token reading "Biography" on a ticket's card would say less than it seems to.
- date: 2026-09-05

### Games — the cover set into the platform's box
- status: confirmed
- decision: A game record is shown as the **physical edition it was met as**: the cover asset is bare key art, and at upload the admin sets it into the box template of the `platform` it was played on (`src/shared/game-box.js` — nine cases: Switch 2, Switch, PS3, Wii U, Wii, GameCube, 3DS, DS, Steam; DS / DS Lite / DSi share the DS case; Steam is an invented case for anything played on a computer, digital console purchases still get the console's box). The composite is **baked as a derivative** (transparent PNG in `cutouts/`, display and thumbnail from it, `?v=` token `bx<template>-<hash>`), the raw art stays the master, and `scripts/rebuild-game-boxes.js` re-derives every box from the same manifest with sharp when a template changes. `platform` becomes a **controlled vocabulary** because it chooses the case; `esrb` (North American rating letter, no descriptors, official icons in `public/game-boxes/esrb/`) fills the case's slot; `cover_fit` records a zoom/offset nudge when cover-fill is wrong. Boxes open like a book, so the front faces are asymmetrical — the hinge side has no edge and the art runs to it; no edge is invented there. Front face only. A SteamGridDB title search in the admin (Netlify proxy, key server-side) supplies key art; developer, year and rating stay hand-entered. On the site a box asset takes the cut-out chain (the case's corners are the silhouette); the boxed-edition CSS rounding remains only for pre-template retail scans.
- reason: Retailer images are inconsistent and often absent for older games, so a consistent look has to be produced rather than found; rendering at upload (not at view time) keeps the site a reader of derivatives like every other cut-out, and keeping the art raw keeps the boxes rebuildable. Templates and ESRB icons are Bay's files; the code owns geometry and compositing only. Mockups: `mockups/game-boxes/`.
- date: 2026-09-11

### Games on the desk — a cartridge, not a save slip
- status: confirmed
- decision: The document standing for **games** in the consumption bundle is a **Switch 2 cartridge at true size** (21 x 31 x 3.4 mm at the desk's `PX_PER_MM`), replacing the save slip that described a game rather than resembling one. It is a **3D object**, not a sheet: its silhouette is geometry — a rounded rectangle extruded to the card's thickness (`cardGeometry()` in `desk-scene.js`, reached by `object: "card"` on a document) — so it takes the lamp along its edges, casts the shadow of a solid, and rises into the fan as a card among papers. Its face is composed the way a game box is (see "Games — the cover set into the platform's box"): the most recent game's **bare key art** cover-fitted into the label window of Bay's cartridge template (`public/desk/switch2-cartridge.webp`, a template with the label cut out), the shell over it, and the record's **accession number overprinted on the serial strip** in place of the printed `LB-XXXXX-XXX-0`. A document may now be `bare` (no stock, no grain — it is moulded, not made of paper), may ask for `fit: "cover"` on an image layer, and may ask for more texels with `texScale`. The label art comes from a new desk-sized derivative, `display/<base>-art-desk.webp`, cropped to the window and written by `scripts/publish-desk-assets.js --art`, with the master as fallback.
- reason: Every other document in the bundle is the thing itself — the log is a log, the receipt a receipt, the sleeve a sleeve — and only games was a description of one. A cartridge is small enough to read as an object among sheets at its true size, and the template Bay drew carries a label window, so the object can say *which* game without a word of text. Making it solid rather than flat costs one geometry function and removes the alpha-masking the flat version would have needed.
- amendment (same day): a card drawn at desk size and a card magnified into the fan want different thicknesses, because the sheets beside it in the fan are planes and it is the only solid among them. `handDepth` on a document eases its depth alone down as it rises and back as it lands; the desk keeps the real 3.4 mm.
- consequence: The games sheet in the opened consumption fan is now a small card — measured at 55 x 81 px on a desktop and 20 x 28 px on a phone, under the 44 px touch floor, so **fan buttons now grow their hit area to 44 px** while the drawn sheet and the one-scale rule stay as they are. That fixes the identity bundle's biography slip at the same time.
- date: 2026-09-11

### Music on the desk — a compact disc
- status: confirmed
- decision: The document standing for **music** in the consumption bundle is a **compact disc at true size** — 120 mm across, a 15 mm spindle, 1.2 mm thick — replacing the record sleeve with a drawn vinyl peeking from behind it. Like the games cartridge it is a **3D object**: `discGeometry()` in `desk-scene.js` (reached by `object: "disc"`) extrudes a circle with the spindle pushed into `shape.holes`, so the hole is real and the desk shows through it. Its face is drawn by one new `cd` layer in `paper.js`: the most recent album's cover printed on the ring from 36 to 118 mm, the clear mirror band inside it, and the **accession number stamped around that band** where a pressed disc carries its matrix and catalogue codes. The vinyl `disc` layer goes out with the sleeve it served, as the procedural `strip` case went out with the drawn film strip. **The change is the desk only**: the browse grid, the catalog card and `dimensions` keep the 12″ vocabulary of `docs/music-display-plan.md`, where the exception is now recorded.
- reason: The same argument as the cartridge — every other document in the bundle is the thing itself, and music was a description of one. A disc needs no new derivative, because a music cover's display copy *is* the art; and at a hundredth of its diameter the edge needs no thinning in hand, unlike the card.
- amendment (same day): the album's title is written across the label in **silver marker**, skewed and set up and to the left of the spindle rather than centred on it — a new `marker` layer in `paper.js` that strokes as well as fills the glyphs (nib at 8% of the size) and inks them with a gradient across the stroke, because metallic is not one colour. Nothing is laid underneath: the ink sits straight on the disc. It takes NO `inkDropout` either — that composites destination-out, and a hole punched in a solid's face shows the shell material through it. **The silver is a surface, not a colour**: `renderSurface()` writes the same strokes into two data canvases — roughness in the green channel, specular strength as a grey — which `desk-scene.js` hangs on the face as `roughnessMap` and `specularColorMap`, with the scalars moved to 1 so the material away from the ink is unchanged. The ink is far smoother and more reflective than the lacquer around it, so it takes the flashlight as a flare where the printed label stays matte. Placed left, the creation sheet covers most of it on the desk and the fan shows it whole; past about x = 0.47 of the diameter it clears that sheet again.
- consequence: At true size the disc is the largest object in the bundle, which is at capacity, so it cannot be placed without lying over something. It sits over the labor bundle's specification sheet and blueprint — documents that carry no subcollection (`sub: null`) — rather than over any of consumption's own, and it clears the books receipt's printed lines. The desk lamp's specular had to be spread (clearcoat 0.75 rather than a truer 0.95) or it washed out a third of the label.
- date: 2026-09-11

### Books on the desk — a library due-date slip
- status: confirmed
- decision: The document standing for **books** in the consumption bundle is a **library pocket slip at true size** — 75 x 125 mm on **soft yellow card** (`legal` stock, #f0e6b4) — replacing the thermal receipt that priced three titles at 12.00 each. It is printed the way a slip is: a `DATE DUE` heading, a rule under it, **six ruled lines** and a column rule dividing a date column from a title column. The **last five books read** fill it live: each date stamped in the red date-stamp face in **MM/DD/YY**, the wheels a date stamp actually carries (`stampBox`, rotated, its own nudge and speckle seed per row), and the title written beside it in the **log's own hand** (Homemade Apple), so the two documents that list books are in one handwriting. Six lines are printed and five are written on: a form is printed before it is used, and the sixth falls under the disc. `stampBox` now takes `size`, `padX`, `boxH`, `lineWidth`, `letterSpacing` and its own dropout seed, because five stamps in a 36 px column cannot be the 9 px office stamp the labor sheet carries; the defaults are unchanged, so that sheet is untouched.
- reason: A receipt is a description of books bought; a due-date slip is the record of books **read**, which is what the subcollection holds. It is also the only document in the bundle that carries dates on its face — the archive keeping its reading the way a library keeps its lending.
- consequence: At 75 mm the two columns need the whole card, so the slip could not stay at the receipt's x: a stamp half-covered by the films log is a stamp that cannot be read. It sits about 23 mm further right, its printed margin still under the log's edge, and higher in the bundle than the receipt sat, so all three stamped rows clear the disc — a solid rides on its **whole thickness**, so the disc's face is above this sheet however early its slot is. What the disc covers is the slip's unused half, which is the right half to lose. Titles are truncated at 22 characters and lose a trailing series parenthetical, because the hand has a 55 px line and two of them per row.
- date: 2026-09-11

### Coffee on the desk — a cupping form
- status: confirmed
- decision: The document standing for **coffee** in the consumption bundle is a **cupping form at true size** (90 x 115 mm, white), replacing the thermal café receipt that priced three coffees at 4.50 each. Roaster and coffee are set in serif at the head with origin, process and date in mono opposite; five short scales — aroma, acidity, body, finish, overall — run on the SCA's 6 to 10, each with a pencilled mark and the score written beside it, and the record's note is written in pencil at the foot. The five scores are **new fields on the coffee record** (`cupping: true` on the `bag` type; `cuppingFields()` gives the admin its own Cupping group) and are `RECORD_ONLY` — three card slots are already spoken for, and a score reads as a form, not as a catalog row. **An unscored bag prints an unmarked form**: a blank form is a truthful document, an invented mark is not.
- reason: A receipt is a description of coffee bought; a cupping form is the record of coffee **tasted**, which is what the subcollection holds. It is also the first document on the desk that carries a judgement rather than a transaction. A café tab was drawn as the alternative and is kept in `mockups/coffee/` — it needed a café-visit record type the archive does not have, so its drink line would have been typeset dressing.
- consequence: The ring is a **real one**: Bay's scan, cut out, kept at `public/desk/coffee-ring.webp` with its alpha. A new `stainImage` layer multiplies it onto the sheet, and it takes the **ring's** outside diameter (65 mm, a saucer) plus a `ring` fraction saying how much of the cropped image the ring spans — 0.9018 here, the rest being splatter — so the scale is right whatever the crop. The drawn `stain` case stays for documents that want a suggestion rather than a photograph. A `crumple` layer was written alongside it for the café tab (a network of creases, each drawn as a highlight and a shadow and blurred, because a hard line reads as pen); the form does not use it. On the desk the films log covers everything but the form's left edge, so the ring sits on the bottom LEFT corner where it can be seen, and the form moved 9 mm further left to widen that strip — the whole left column of scale labels now reads from across the room, and the fan shows the form entire.
- date: 2026-09-12
