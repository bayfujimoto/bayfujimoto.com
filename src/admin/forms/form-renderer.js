...
import { isFoldableType } from "../../shared/field-schema.js";
...
function makeInspectionAwareAssets(mode, currentData, handleChange, getItemId) {
  ...
  if (mode === "card") {
    ...
    // Folded matter (ephemera types): the open state's two faces, optional.
    // Uploading either is what makes the record folded — the card then shows
    // unfold / fold. Never the record thumbnail (that stays the closed recto).
    // Scanned on the carrier sheet like the closed faces, so cut-out is offered.
    // docs/brochure-fold-states-plan.md.
    if (isFoldableType(currentData.item_type)) {
      const insideField  = { id: "assets.inside",  label: "inside (open, optional)",  type: "asset-upload", assetRole: "inside",  allowCutout: true, skipThumbnail: true };
      ...
      container.appendChild(makeAssetUploadField(insideField,  currentData.assets?.inside,  handleChange, getItemId));
      container.appendChild(makeAssetUploadField(outsideField, currentData.assets?.outside, handleChange, getItemId));
    }
    return container;
  }
