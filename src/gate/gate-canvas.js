...
//
// Same Bayer 4x4 ordered-dither matrix the admin uses elsewhere. WIN=0.5
// per-cell window. Phase is radial from the touch point: cells nearest the
// press start immediately, cells at the far corner start at (1 - WIN), all
// finish by progress = 1.
//
// Recolored to the admin palette (src/styles/tokens.css [data-theme="admin"]):
// ink = --fg #1f2226 (charcoal), paper = --bg #d5d8db (cool grey), so the gate
// reads as continuous with the admin it protects, not the public archive.
...
// Ink = admin --fg (charcoal); paper = admin --bg (cool grey).
const INK = [0x1f, 0x22, 0x26];
const PAPER = [0xd5, 0xd8, 0xdb];
...
export function initGate({ onSuccess, onReset } = {}) {
  ...
  // A pixel is inked iff its Bayer threshold falls under the cell's brightness.
  ...
  function fillPaper() {
    for (let i = 0; i < imageData.data.length; i += 4) {
      imageData.data[i]     = PAPER[0];
      imageData.data[i + 1] = PAPER[1];
      imageData.data[i + 2] = PAPER[2];
      imageData.data[i + 3] = 255;
    }
    ctx.putImageData(imageData, 0, 0);
  }
